  ...
  class PhaseVocoder {
    ...
    stft(inputFrame, windowFrame, processedFrame, fftObj) {
      const winSize = this.winSize;
      const fft = this.fft;
  ...
  class AudioC {
    ...
    #decodeBuffer(audioData) {
      return new Promise(resolve => {
        ...
        let buffer;
        try {
          buffer = this.ctx.createBuffer(1, decodedData.length, 8000);
        } catch (ignore) {
          // iOS 不支持 22050 以下的采样率，于是先提升采样率，然后用慢速播放
          if (this.sampleRate < 11025) {
            buffer = this.ctx.createBuffer(1, decodedData.length, 8000 * 4.0);
            this.playbackRate = 0.25;
          } else {
            buffer = this.ctx.createBuffer(1, decodedData.length, 8000 * 2.0);
            this.playbackRate = 0.5;
          }
        } finally {
          (buffer && buffer.copyToChannel) ? buffer.copyToChannel(decodedData, 0, 0): buffer.getChannelData(0).set(decodedData);
        }
	buffer = this.getWavData(buffer).buffer;
        return resolve(buffer);
      }).then(buffer => {
        // if (!!buffer) return buffer;
	buffer = this.rawData = buffer || audioData;
