...
var _amrnb = _interopRequireDefault(require("./amrnb.js"));
...
var _default = function (root, factory) {
  ...
  var AMRDecoder = /*#__PURE__*/function () {
    ...
    _createClass(AMRDecoder, [{
      ...
      value: function init() {
        // Create decoder
        this.state = _amrnb["default"].Decoder_Interface_init();

        // 'XXX' - change to parameters

        // Input Buffer
        this.input = _amrnb["default"].allocate(new Int8Array(20), 0);

        // Buffer to store the audio samples
        this.buffer = _amrnb["default"].allocate(new Int16Array(160), 0);
      }
    }, {
      key: "validate",
      ...
      value: function read(offset, data) {
        ...
        for (var m = offset - 1, k = 0, bits; ++m < offset + len; k += 1) {
          ...
          _amrnb["default"].setValue(input_addr + k, bits, 'i8');
        }
        return len;
      }
    }, {
      ...
      value: function process(data) {
        ...
        while (offset < data.length) {
          ...
          // Decode the data
          _amrnb["default"].Decoder_Interface_Decode(state_addr, input_addr, buffer_addr, 0);
      ...
      value: function write(offset, nframes, addr) {
        for (var m = 0, k = offset - 1; ++k < offset + nframes; m += 2) {
          this.output[k] = _amrnb["default"].getValue(addr + m, "i16") / 32768;
        }
      }
    }, {
      key: "close",
      value: function close() {
        _amrnb["default"].Decoder_Interface_exit(this.state);
      }
    }]);
    return AMRDecoder;
  }();
  var AMREncoder = /*#__PURE__*/function () {
    ...
    _createClass(AMREncoder, [{
      ...
      value: function init() {
        // Create Encoder
        this.state = _amrnb["default"].Encoder_Interface_init(this.dtx);
        this.input = _amrnb["default"].allocate(new Int16Array(this.frame_size), 0);
        this.buffer = _amrnb["default"].allocate(new Int8Array(this.block_size), 0);
      }
    }, {
      key: "read",
      value: function read(offset, length, data) {
        ...
        for (var m = offset - 1, k = 0; ++m < offset + len; k += 2) {
          _amrnb["default"].setValue(input_addr + k, data[m], 'i16');
        }
        return len;
      }
    }, {
      ...
      value: function write(offset, nb, addr) {
        for (var m = 0, k = offset - 1; ++k < offset + nb; m += 1) {
          this.output[k] = _amrnb["default"].getValue(addr + m, 'i8');
        }
      }
    }, {
      key: "process",
      value: function process(pcmdata) {
        ...
        while (offset < pcmdata.length) {
          ...
          // Encode the frame
          nb = _amrnb["default"].Encoder_Interface_Encode(this.state, this.mode, input_addr, buffer_addr, 0);
      ...
      key: "close",
      value: function close() {
        _amrnb["default"].Encoder_Interface_exit(this.state);
      }
    }]);
    return AMREncoder;
  }();
